...
// Check if sourceFile1 exists and is readable
fs.access(sourceFile1, fs.constants.R_OK, (err) => {
  if (err) {
    console.error(`Error reading ${sourceFile1}: ${err.message}`);
    process.exit(1);
  }

  // Check if sourceFile2 exists and is readable
  fs.access(sourceFile2, fs.constants.R_OK, (err) => {
    if (err) {
